...
def make_batch(signal_batch, noise_amount, scaling_factor, permutation_pieces, time_warping_pieces,
    ...
    for i in range(len(signal_batch)):
        ...
        noised_signal = stt.add_noise_with_SNR(signal,
                                               noise_amount=noise_amount)  # round(np.random.uniform(0.005,0.05),2)) # 0.005 - 0.05
        ...
        time_warped_signal = stt.time_warp(signal, sampling_freq, pieces=time_warping_pieces,
                                           stretch_factor=time_warping_stretch_factor,
                                           squeeze_factor=time_warping_squeeze_factor)

        ## making signals of same size..
        tw_start_index = int(np.random.randint(0, (len(time_warped_signal) - len(signal))))
        tw_stop_index = int(tw_start_index + len(signal))
        ...
        batch = [signal, noised_signal, scaled_signal, negated_signal, flipped_signal, permuted_signal,
                 time_warped_signal]
