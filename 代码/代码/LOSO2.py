...
tw_piece_param = 9
twsf_param = 1.06
no_of_task = ['original_signal', 'noised_signal', 'scaled_signal', 'negated_signal', 'flipped_signal',
              'permuted_signal', 'time_warped_signal']
...
##subject_ids_to_run = subject_ids[:2]
##total_subjects = len(subject_ids_to_run)
...
acc, f1 = my_model.supervised_model_wesad(
...
def plot_history(history, subject_id, output_dir):
    ...
    plt.title(f'留出受试者: {int(subject_id)} 损失曲线')
    plt.xlabel('Epoch')
    plt.ylabel('损失')
    plt.legend()
    ...
    plt.title(f'留出受试者: {int(subject_id)} 准确率曲线')
    plt.xlabel('Epoch')
    plt.ylabel('准确率')
    plt.legend()
    ...
    # 绘制F1分数曲线
    plt.subplot(1, 3, 3)
    plt.plot(history['train_f1'], label='训练F1分数')
    plt.plot(history['val_f1'], label='验证F1分数')
    plt.title(f'留出受试者: {int(subject_id)} F1分数曲线')
    plt.xlabel('Epoch')
    plt.ylabel('F1分数')
    plt.legend()
...
def plot_downstream_history(history, save_path):
    """
    绘制下游任务的训练和验证的损失、准确率和F1曲线图。
    """
    plt.figure(figsize=(18, 5))
    ...
    plt.title('Downstream Task Loss Curve')
    plt.xlabel('Epoch')
    plt.ylabel('loss')
    plt.legend()
...
print("\n正在生成下游任务的性能曲线图...")
downstream_figure_dir = os.path.join(output, "STR_result", "figures")
